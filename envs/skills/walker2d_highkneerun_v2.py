...
class Walker2dHighKneeRun2Env(Walker2dEnv):
    def __init__(self):
        super().__init__()
        ...
        self._config.update({
            ...
            "angle_reward": 0.1,
            "foot_reward": 0.01,
            "height_reward": 5, # change from 1 to 5
            "x_vel_limit": 3,
        })
    ...
    def step(self, a):
        ...
        # change definition of delta_h
        delta_h = self.data.body_xpos[1, 2] - max(self.data.body_xpos[3, 2], self.data.body_xpos[5, 2])
        nz = np.cos(angle)
        ...
        height_reward = -self._config["height_reward"] * abs(delta_h) # change height_reward
